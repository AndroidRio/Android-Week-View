...
public class WeekView extends View {
    private Locale mBrazilLocale = new Locale("pt", "BR");
    @Deprecated
    ...
    private Paint mHeaderDayWeekTextPaint;
    private float mHeaderTextHeight;
    private float mHeaderWeekDayWidth;
    ...
    private Paint mTodayHeaderDayWeekTextPaint;
    private Paint mEventBackgroundPaint;
    private Paint mTopLeftPaint;
    ...
    private int mHeaderTextSize = 70;
    private int mHeaderColumnPadding = 10;
    private int mHeaderColumnTextColor = Color.BLACK;
    private int mNumberOfVisibleDays = 3;
    private int mHeaderRowPadding = 10;
    private int mHeaderRowBackgroundColor = Color.WHITE;
    private int mDayBackgroundColor = Color.argb(0, 245, 245, 245);
    ...
    private String mTimeFormat = "00:00";
    private String mWeekDayFormat = "www";
    ...
    public WeekView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            mHeaderTextSize = a.getDimensionPixelSize(R.styleable.WeekView_dayTextSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, mTextSize, context.getResources().getDisplayMetrics()));
    ...
    private void init(Context context) {
        ...
        Rect rect = new Rect();
        mTimeTextPaint.getTextBounds(mTimeFormat, 0, mTimeFormat.length(), rect);
        mTimeTextWidth = mTimeTextPaint.measureText(mTimeFormat);
        ...
        mHeaderTextPaint.getTextBounds(mTimeFormat, 0, mTimeFormat.length(), rect);
        mHeaderTextHeight = rect.height();
        mHeaderTextPaint.setTypeface(Typeface.DEFAULT);

        // Prepare today header text color paint.
        mTodayHeaderTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mTodayHeaderTextPaint.setTextAlign(Paint.Align.LEFT);
        mTodayHeaderTextPaint.setTextSize(mHeaderTextSize);
        mTodayHeaderTextPaint.setTypeface(Typeface.DEFAULT);
        mTodayHeaderTextPaint.setColor(mTodayHeaderTextColor);

        //Prepare today week day text paint.
        mTodayHeaderDayWeekTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mTodayHeaderDayWeekTextPaint.setColor(mTodayHeaderTextColor);
        mTodayHeaderDayWeekTextPaint.setTextAlign(Paint.Align.LEFT);
        mTodayHeaderDayWeekTextPaint.setTextSize(mTextSize);
        mTodayHeaderDayWeekTextPaint.getTextBounds(mWeekDayFormat, 0, mWeekDayFormat.length(), rect);
        mHeaderWeekDayWidth = rect.width();
        mTodayHeaderDayWeekTextPaint.setTypeface(Typeface.DEFAULT);

        //Prepare week day text paint.
        mHeaderDayWeekTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mHeaderDayWeekTextPaint.setColor(mHeaderColumnTextColor);
        mHeaderDayWeekTextPaint.setTextAlign(Paint.Align.LEFT);
        mHeaderDayWeekTextPaint.setTextSize(mTextSize);
        mHeaderDayWeekTextPaint.setTypeface(Typeface.DEFAULT);
        ...
        // Prepare top left corner color paint;
        mTopLeftPaint = new Paint();
        mTopLeftPaint.setColor(mDayBackgroundColor);
        ...
        // Prepare event marker width
        mMarkerWidth = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 2, context.getResources().getDisplayMetrics());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawRect(0, 0, mTimeTextWidth + mHeaderColumnPadding * 2 - 1, mHeaderTextHeight + mHeaderRowPadding * 2 - 1, mTopLeftPaint);
...
//                canvas.drawText(time, mTimeTextWidth + mHeaderColumnPadding, top + mTimeTextHeight, mTimeTextPaint);
                canvas.drawText(time, mTimeTextWidth + mHeaderColumnPadding, top + mTimeTextHeight/2, mTimeTextPaint);
        }
    }
...
//                canvas.drawRect(start, mHeaderTextHeight + mHeaderRowPadding * 2 + mTimeTextHeight / 2 + mHeaderMarginBottom, startPixel + mWidthPerDay, getHeight(), sameDay ? mTodayBackgroundPaint : mDayBackgroundPaint);
                canvas.drawRect(start, mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom, startPixel + mWidthPerDay, getHeight(), sameDay ? mTodayBackgroundPaint : mDayBackgroundPaint);
...
//                float top = mHeaderTextHeight + mHeaderRowPadding * 2 + mCurrentOrigin.y + mHourHeight * hourNumber + mTimeTextHeight / 2 + mHeaderMarginBottom;
                float top = mHeaderTextHeight + mHeaderRowPadding * 2 + mCurrentOrigin.y + mHourHeight * hourNumber + mHeaderMarginBottom;
//                if (top > mHeaderTextHeight + mHeaderRowPadding * 2 + mTimeTextHeight / 2 + mHeaderMarginBottom - mHourSeparatorHeight && top < getHeight() && startPixel + mWidthPerDay - start > 0) {
                if (top > mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom - mHourSeparatorHeight && top < getHeight() && startPixel + mWidthPerDay - start > 0) {
                    hourLines[i * 4] = start;
                    hourLines[i * 4 + 1] = top;
...
//            String dayLabel = getDateTimeInterpreter().interpretDate(day);
            String dayMonth = String.valueOf(day.get(Calendar.DAY_OF_MONTH));
            String weekDay = day.getDisplayName(Calendar.DAY_OF_WEEK, Calendar.SHORT, mBrazilLocale);

            if (dayMonth == null || weekDay == null)
                throw new IllegalStateException("A Calendar must not return null date");

            weekDay = weekDay.toLowerCase();
            //TODO Tratar o caso de o modo selecionado ser de 1 dia só. Existe um problema de design. É preciso determinar se o dia (número) se move no scroll ou não.
//            if(mNumberOfVisibleDays == 1){
//                canvas.drawText(dayMonth, startPixel - mTimeTextWidth + mHeaderColumnPadding, mHeaderTextHeight + mHeaderRowPadding, sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
//                canvas.drawText(weekDay, startPixel + mWidthPerDay - mHeaderRowPadding - mHeaderWeekDayWidth, mHeaderTextHeight / 2.5f + mHeaderRowPadding, sameDay ? mTodayHeaderDayWeekTextPaint : mHeaderDayWeekTextPaint);
//            }else {
                canvas.drawText(dayMonth, startPixel + mHeaderRowPadding, mHeaderTextHeight + mHeaderRowPadding, sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
                canvas.drawText(weekDay, startPixel + mWidthPerDay - mHeaderRowPadding - mHeaderWeekDayWidth, mHeaderTextHeight / 2.5f + mHeaderRowPadding, sameDay ? mTodayHeaderDayWeekTextPaint : mHeaderDayWeekTextPaint);
//            }

            // Draw the days separator lines
            if(dayNumber == 1)
                canvas.drawLine(startPixel, 0, startPixel, getHeight(), mHourSeparatorPaint);
            canvas.drawLine(startPixel + mWidthPerDay, 0, startPixel + mWidthPerDay, getHeight(), mHourSeparatorPaint);

            startPixel += mWidthPerDay + mColumnGap;
        }

        // Draw header row separator line
        canvas.drawLine(0, mHeaderTextHeight + mHeaderRowPadding * 2  + mHeaderMarginBottom,getWidth(),mHeaderTextHeight + mHeaderRowPadding * 2  + mHeaderMarginBottom,mHourSeparatorPaint);
...
//                    float top = mHourHeight * 24 * mEventRects.get(i).top / 1440 + mCurrentOrigin.y + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 + mEventMarginVertical;
                    float top = mHourHeight * 24 * mEventRects.get(i).top / 1440 + mCurrentOrigin.y + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mEventMarginVertical;
                    ...
                    if (top < mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom)
                        top = mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom;
...
//                    bottom = mHourHeight * 24 * bottom / 1440 + mCurrentOrigin.y + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 - mEventMarginVertical;
                    bottom = mHourHeight * 24 * bottom / 1440 + mCurrentOrigin.y + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom - mEventMarginVertical;
...
//                    if (bottom > mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 && left < right &&
                    if (bottom > mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom && left < right &&
...
//                            eventRectF.bottom > mHeaderTextHeight + mHeaderRowPadding * 2 + mTimeTextHeight / 2 + mHeaderMarginBottom &&
                            eventRectF.bottom > mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom &&
                            eventRectF.top < getHeight() &&
                            left < right
                            ) {
                        mEventRects.get(i).rectF = eventRectF;
                        RectF markerRectF = new RectF(left, top, left + mMarkerWidth, bottom);
                        ...
                        canvas.drawRect(markerRectF, mEventBackgroundPaint);

                        int alphaColor = Color.argb(100, Color.red(color), Color.green(color), Color.blue(color));
                        mEventBackgroundPaint.setColor(alphaColor);
                        canvas.drawRect(mEventRects.get(i).rectF, mEventBackgroundPaint);



                        drawText(mEventRects.get(i).event.getName(), mEventRects.get(i).rectF, canvas, originalTop, originalLeft, color);
                    } else
     ...
     */
    private void drawText(String text, RectF rect, Canvas canvas, float originalTop, float originalLeft, int color) {
